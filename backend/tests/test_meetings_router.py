...
def _seed_booking(db, lead_id, *, when=None):
    ...
    db.execute(
        ...
        ),
        {
            "id": str(bid),
            "lid": str(lead_id),
            "when": when or datetime.now(timezone.utc) + timedelta(hours=4),
        },
    )
    db.commit()
    return bid
...
def test_list_returns_paginated_meetings(client, db, leads_db, admin_token):
    ...
    res = client.get(
        "/admin/meetings", headers={"Authorization": f"Bearer {admin_token}"}
    )
...
def test_detail_returns_booking_lead_empty_notes(client, db, leads_db, admin_token):
    ...
    leads_db.execute(
        text(
            "INSERT INTO leads (id, first_name, last_name, institution, stage, lead_score) "
...
def test_detail_not_found(client, admin_token):
    res = client.get(
        f"/admin/meetings/{uuid4()}", headers={"Authorization": f"Bearer {admin_token}"}
    )
...
def test_patch_notes_updates_prep_and_recap(
    client, db, leads_db, operator_token, admin_token
):
    lid = uuid4()
    leads_db.execute(
        text("INSERT INTO leads (id, first_name, last_name) VALUES (:id, 'P', 'Q')"),
        {"id": str(lid)},
    )
    leads_db.commit()
    bid = _seed_booking(db, lid)
...
def test_notes_patch_requires_operator(client, db, leads_db):
    ...
    lid = uuid4()
    leads_db.execute(
        text("INSERT INTO leads (id, first_name, last_name) VALUES (:id, 'R', 'O')"),
        {"id": str(lid)},
    )
    leads_db.commit()
    bid = _seed_booking(db, lid)
...
def test_task_create_update_delete(client, db, leads_db, operator_token, admin_token):
    lid = uuid4()
    leads_db.execute(
        text("INSERT INTO leads (id, first_name, last_name) VALUES (:id, 'T', 'D')"),
        {"id": str(lid)},
    )
    leads_db.commit()
    bid = _seed_booking(db, lid)
...
def test_task_404_when_wrong_booking(client, db, leads_db, operator_token):
    lid = uuid4()
    leads_db.execute(
        text("INSERT INTO leads (id, first_name, last_name) VALUES (:id, 'A', 'C')"),
        {"id": str(lid)},
    )
    leads_db.commit()
    bid = _seed_booking(db, lid)
    ...
    other = uuid4()
    res = client.patch(
        f"/admin/meetings/{other}/tasks/{task_id}",
        json={"done": True},
        headers={"Authorization": f"Bearer {operator_token}"},
    )
    assert res.status_code == 404
